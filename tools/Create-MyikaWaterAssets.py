...
def build_master_material(normal_texture):
	...
	# 2026-05-05 (Claude, headless verification): originally this raised on
	# False, but `recompile_material` returns False when it could not TRIGGER
	# a recompile — not when the compile failed. Compile is async; failures
	# show up later in OutputLog as `Failed to compile Material for platform
	# PCD3D_SM6`. Continue + save the asset so MIs can be authored against it,
	# and reflect the recompile-trigger result in the summary so downstream
	# can decide whether to act on it.
	if not recompile_ok:
		unreal.log_warning(
			f"[MyikaWater] recompile_material returned False for {material.get_path_name()}. "
			"Recompile could not be triggered. Check OutputLog for the underlying SM6/SM5 "
			"error after this run completes."
		)
	save_asset(material)
