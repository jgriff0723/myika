...
def write_summary(texture, material) -> None:
	...
	for expression in unreal.MaterialEditingLibrary.get_material_expressions(material_asset):
		if isinstance(expression, unreal.MaterialExpressionTextureSampleParameter2D):
			texture_sampler = expression
			break
	...
	data = {
		...
		"texture_sampler_type": enum_name(texture_sampler.get_editor_property("sampler_type")) if texture_sampler else None,
	}
