...
private:
	...
	// Re-entry guard — same pattern as MyikaOcean / MyikaLake / MyikaSky.
	// RefreshRiver() is called from OnConstruction *and* PostEditChangeProperty,
	// and inside it SetWaterMaterial / SynchronizeAndBroadcastDataChange /
	// decal RegisterComponent can each fire another property change in the
	// editor and re-enter. Without the guard, opening the river actor in the
	// editor blows the stack.
	bool bRefreshingRiver = false;
};
