...
// HideCategories: the engine subobject components (SkyAtmosphere / VolumetricCloud /
// SkyLight / ExpHeightFog / PostProcess / etc.) each carry hundreds of nested UPROPERTY
// structs. Inlining all of them under the actor's details panel makes the Slate property
// tree recurse deep enough to overflow the editor stack — see crash UECC ...4ADE812553AC...
// (`SDocumentationToolTip::AddDocumentation` -> `__chkstk`). Components remain editable via
// the Components panel; users still see / scrub time-of-day, sun, cloud preset, curves, etc.
UCLASS(BlueprintType, Blueprintable, ClassGroup = (Myika),
	HideCategories = ("Myika|Sky|Components"),
	meta = (DisplayName = "Myika Sky Actor"))
...
private:
	...
	// Re-entry guards: VolumetricCloudComponent->SetMaterial() can fire
	// PostEditChangeProperty / re-trigger OnConstruction in the editor, which
	// then re-enters these helpers. Without guards the recursion blows the
	// editor stack the moment AMyikaSkyActor is selected (same pattern fixed
	// for AMyikaOcean — see MyikaOceanActor.h).
	bool bApplyingCloudMaterial = false;
	bool bEnsuringCloudMaterialInstance = false;
};
